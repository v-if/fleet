...
## 3. Vercel 배포 오류 분석 (fleet-tau.vercel.app)
...
| **Phase 3 Tesla 연동 배포 테스트** | OAuth 토큰·동기화·Mock 폴백을 `fleet-tau.vercel.app`에서 검증 |
...
| **Vercel Production** | Supabase dev(초기) → 추후 production 프로젝트(Phase 5) | `fleet-tau.vercel.app` |
...
| `NEXT_PUBLIC_KAKAO_MAP_KEY` | 지도용 | Kakao에 `fleet-tau.vercel.app` 도메인 등록 |
