...
- Vercel 배포(`https://bori-fleet.vercel.app`): UI·API **200**, mock·tesla 연동 검증 완료 (2026-07-07)

---

## 3. Vercel 배포 오류 분석 (bori-fleet.vercel.app)

### 3.1 증상

| 구분 | 로컬 (`localhost:3000`) | Vercel (`bori-fleet.vercel.app`) |
|------|-------------------------|----------------------------------|
...
| **Phase 3 Tesla 연동 배포 테스트** | OAuth 토큰·동기화·Mock 폴백을 `bori-fleet.vercel.app`에서 검증 |
...
| **Vercel Production** | Supabase dev(초기) → 추후 production 프로젝트(Phase 5) | `bori-fleet.vercel.app` |
...
| `NEXT_PUBLIC_KAKAO_MAP_KEY` | 지도용 | Kakao에 `bori-fleet.vercel.app` 도메인 등록 |

#### 로컬 `.env`

- `DATABASE_URL`을 Supabase dev URL로 교체 (SQLite `file:./dev.db` 대체)
- 기존 Tesla·Kakao 변수 유지
...
- [x] `https://bori-fleet.vercel.app/api/vehicles` → **HTTP 200** + 차량 JSON (2026-07-07)
