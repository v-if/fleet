...
> **DB 설계**: OAuth Access/Refresh 토큰은 **차량 단위가 아니라 테슬라 계정 단위**로 관리한다. `GET /api/1/vehicles`는 계정에 묶인 차량 배열을 반환한다. 상세: [requirements-user-db.md](./requirements-user-db.md) §3.1.
...
- OAuth 연결만으로 Fleet API 조회는 불가 — **Partner Register**(Phase 3.5) 필요. 미완료 시 `412` 오류.

### 2.5 Partner Register (412 대응) — Phase 3.5

OAuth(Third-party token)와 Partner Register는 **별개**다.

| 구분 | Third-party token (Phase 3) | Partner token + Register (Phase 3.5) |
|------|----------------------------|--------------------------------------|
| 발급 | `authorization_code` (사용자 로그인·동의) | `client_credentials` (앱 자격증명) |
| 용도 | 사용자 대신 API 접근 권한 위임 | **앱을 Fleet API 리전에 등록** |
| UI 표시 | `/settings` “계정 연결됨” | 별도 UI 없음 (백엔드 1회 작업) |
| localhost | OAuth redirect 가능 | **register 불가** (공개 HTTPS 도메인 필요) |

#### 412 오류 예시
```json
{
  "error": "Account {client_id} must be registered in the current region https://fleet-api.prd.na.vn.cloud.tesla.com, please see .../partner-endpoints#register"
}
```

#### Register 절차 요약
1. **EC 키 쌍** 생성 (secp256r1)
2. **공개키 HTTPS 호스팅**
   ```
   https://{domain}/.well-known/appspecific/com.tesla.3p.public-key.pem
   ```
3. **Partner 토큰** 발급 (`POST` `https://fleet-auth.prd.vn.cloud.tesla.com/oauth2/v3/token`)
   - `grant_type=client_credentials`
   - `client_id`, `client_secret`, `audience` (= Fleet API base URL, 한국=`na`)
   - `scope`: 앱에 등록된 스코프 (예: `openid vehicle_device_data vehicle_location offline_access`)
4. **Register 호출** (한국 → `na` 리전)
   ```
...
- 진행 현황(2026-07-07): EC 키 쌍 생성, `/.well-known/appspecific/com.tesla.3p.public-key.pem` 배포 확인, Partner token 발급, `fleet-tau.vercel.app` 도메인 Register 및 public key 조회 확인 완료
...
> TPMS 메모: Tesla Fleet API의 `tpmsFrontLeft` 등 수치는 국내에서 익숙한 PSI가 아니라 atm(또는 bar에 준하는 압력 단위) 기준으로 간주한다. FMS 화면 표시는 `1 atm ≒ 14.7 PSI`를 곱해 PSI로 환산한다.
...
| **Partner Register** | OAuth만으로는 `GET /api/1/vehicles` 등 조회 불가 → **412** | Phase 3.5: 공개 도메인·공개키·`POST /partner_accounts` ([§2.5](./requirements-tesla-api.md)) |
...
| 2026-07-07 | §2.5 Partner Register(412) 추가 — OAuth vs Register 구분, Phase 3.5 근거 |
| 2026-07-07 | Phase 3.6 로컬 DB 완료 반영 — Register 선행 조건(Vercel API) 명시 |
| 2026-07-07 | Phase 3.5 진행 상태 반영 — EC 키 생성·공개키 파일 배치 완료 |
| 2026-07-07 | Phase 3.5 Register 진행 반영 — 공개키 URL 확인, Partner Register·public_key 조회 성공 |
| 2026-07-07 | Tesla 연동 검증 반영 — `fleet_status` 파싱 오류 수정, `usedFallback=false` 확인 |
| 2026-07-07 | Tesla UI 보정 반영 — VIN 기반 식별명, 좌표 `0,0` 안내 처리 |
| 2026-07-07 | Tesla TPMS 표시 보정 — atm(≈bar) → PSI (`× 14.7`) 환산 명시 |
| 2026-07-07 | Phase 3.5·3.6 Vercel 배포 검증 완료 — mock·tesla 연동, `usedFallback=false` |
| 2026-07-08 | DB 연계 — 계정 단위 토큰·User/TeslaAccount/Vehicle 계층은 [requirements-user-db.md](./requirements-user-db.md) 참고 |
